...
**Re-baselined**: 2026-09-09 (review round 2 on PR #62: the per-stack usage signal is restored as metrics, PR #378's top-level failure handler is absorbed, and every flush goes through one bounded helper. See [review round 2 note](research/2026-09-09-review-round-2.md).)
**Status**: Draft
...
- What happens for a stack that synthesizes to HCL instead of `cdk.tf.json`? The CLI reads only the separate metadata file for those stacks, so `required_providers` is not in hand and **no `cli.stack.provider` metric is emitted** for them (the stack still counts as `cli.stack`). Recorded 2026-09-09 as a known gap; not fixed in this feature.
...
- **FR-003**: `@sentry/node` MUST be upgraded from `7.120.4` to `^10.56` across the three consuming packages (`@cdktn/commons`, `@cdktn/cli-core`, `cdktn-cli`), and usage analytics MUST be emitted via the **v10 metrics API** (`Sentry.metrics.count(...)`). `Sentry.init()` MUST set `tracesSampleRate: 0` — empirically confirmed (spike 2026-06-10, `@sentry/node@10.57.0`) to deliver metrics independently of trace sampling (see FR-015). `enableLogs` is NOT required for metrics and MUST be omitted unless structured logs are independently needed (they are not — YAGNI). The legacy 7.x `Sentry.metrics.metricsAggregatorIntegration()` / `Sentry.metrics.increment()` mechanism MUST NOT be used — the server-side product it targeted was sunset 2024-10-07 and is a silent no-op on `7.120.4` (research Decision D1).
  - *Note 2026-09-09*: the branch pins `^10.73.0` across the three packages. The 10.x metrics API is unchanged from the June spike, and v11 (beta) is not adopted — it buys nothing this feature needs.
...
- **FR-005**: A new `sendUsageTelemetry` field MUST be added to `cdktf.json` to independently control usage telemetry, separate from `sendCrashReports` which controls crash reporting. It MUST be declared on `ConfigBase` in `packages/@cdktn/commons/src/config.ts` (language-agnostic — NOT inside the language-discriminated union), which is what documents it in the `cdktf.json` schema, and it MUST have exactly **one reader**: the forgiving raw reader in `@cdktn/commons` (`getUsageTelemetryConsent`), which the gating step (FR-017) consults. A malformed value is treated there as "not explicitly false", i.e. the legacy default applies. **The consent UX for `sendUsageTelemetry` MUST mirror `sendCrashReports`** (prompt on first CLI use in an interactive terminal, decision persisted to `cdktf.json`; see FR-008). The non-interactive/CI default differs per flag (FR-016). This deliberate alignment of the consent *surface* is the rationale for introducing the new config field — now that usage analytics also flow through Sentry, the prompt mechanism is unified. It is the **only** intended UX change relative to the legacy on-by-default usage telemetry; the CI handling and the `CHECKPOINT_DISABLE` override are otherwise preserved (what is collected is restored and extended by FR-019/FR-020).
  - *Revised 2026-09-09 (review round 2)*: the validated `CdktfConfig.sendUsageTelemetry` getter this requirement originally mandated (`cli-core/src/lib/cdktf-config.ts`, per the `importExtension` precedent) has been **removed**, together with its tests. It had zero production callers: gating lives in commons, which cannot import cli-core and must work before or without a valid project (no `cdktf.json`, pre-validation init, `convert` outside a project — research Decision 8). It also parsed the flag differently from the reader that actually gates emission — it threw `External` on a value the commons reader treats as unset — so the two surfaces could disagree about the same file while only one of them decided anything. Removing it follows the round-2 reviewer themes (do not keep code or tests that buy nothing; one helper per job). The `ConfigBase` declaration stays; contract C2's "two readers by design" clause is superseded.
...
- **FR-012**: The `report()` function in errors.ts that sends error data to HashiCorp via `ReportRequest` MUST be removed, and the `Errors` object and its Sentry integration MUST be preserved. The factories MUST NOT POST anything. In its place, every error built by `Errors.Internal/External/Usage` MUST be counted as `cli.error` with attributes `type` (`Usage`/`External`/`Internal`) and `command` (the scope set by `Errors.setScope`, read **at call time**, not when the factory is created), gated by usage telemetry like every other metric. Neither the error message nor the context values may be attached — both can carry paths and user input; the full error still reaches Sentry as an event when `sendCrashReports` is on (FR-009).
  - *Revised 2026-09-09 (review round 2)*: the original wording removed the HashiCorp report and put nothing back, which silently dropped the error signal the factories used to carry (reviewer thread on dropped `Errors` reporting). `cli.error` counts constructed errors — including handled `Usage` errors that are not crashes — while `cli.command.error` (FR-014) counts failed **runs**; the two count different things and do not double-count.
...
- **FR-014**: The canonical usage metrics MUST be exactly the following. Every one of them is gated by `sendUsageTelemetry`/`CHECKPOINT_DISABLE` (FR-017) and is distinct from crash reporting (FR-009). Except where noted, each carries the **base attribute set**: `command`, `ci`, `os`, `arch`, `binary`, `binary_version` (when the product is recognised), `target_terraform`/`target_opentofu` (when declared or defaulted), `targets_declared`, `validate_installed_binary`, and `language` when it is one of the supported languages (FR-019, FR-021).

  | Metric | Kind / value | Attributes beyond the base set |
  |---|---|---|
  | `cli.command.invoked` | count 1 per command run | the allow-listed per-command scalars (FR-021) |
  | `cli.command.error` | count 1 per failed command run | `error_type` ∈ `Usage`, `External`, `Internal`, `unexpected` |
  | `cli.synth.duration` | distribution, milliseconds | — |
  | `cli.stack` | count 1 per synthesized stack (synth/diff/deploy/destroy) | `backend`, `cloud`, `override_count`, `import_count`, `moved_count`, `library_version` (when it is a release) |
  | `cli.stack.override` | count 1 per overridden resource type | `resource_type`, `override_count` |
  | `cli.stack.provider` | count 1 per required provider | `provider`, `binding`, `version_constraint` (when declared) |
  | `cli.stack.failed` | count = number of stacks that did not complete | — |
  | `cli.get.provider` / `cli.get.module` | count 1 per generated binding | `provider` / `module` |
  | `cli.init.provider` | count 1 per provider passed to `init` | `provider` |
  | `cli.error` | count 1 per error built by the `Errors` factories | `type`, `command` — **and nothing else**: this metric does not carry the base set (FR-012) |

  `cli.command.error` MUST be emitted **once** per failed run: the CLI entrypoint's failure reporter (FR-022) counts anything that reaches it, and the two synth paths that hard-exit the process themselves count their own failure immediately before exiting, because the reporter never sees them. No failure is counted on both paths.
  - *Revised 2026-09-09 (review round 2)*: the original list named three metrics and left the rest of the payload discarded. The per-stack and per-binding metrics restore the signal the HashiCorp payload carried (FR-020), and the double-count that the earlier synth path produced — one `cli.command.error` from `synth-stack` and a second from the entrypoint for the same thrown error — is gone.
...
- **FR-019** *(added 2026-09-09)*: Every command metric MUST carry the environment the command actually ran in: `binary` (`terraform`, `opentofu`, `unknown`, `missing`), `binary_version` (MAJOR.MINOR.PATCH, and only when the probed product is recognised), `target_terraform` and `target_opentofu` (the ranges declared in `cdktf.json` `targetVersions`, else the CLI's `DEFAULT_TARGET_VERSIONS`), `targets_declared`, `validate_installed_binary`, `os` and `arch`. The binary MUST be probed **once per process**, lazily on first use, through a single process-global probe shared across every copy of commons loaded into the bundle, and the wait for it MUST be bounded (1500 ms; a probe that does not settle in time reports `binary: "unknown"`) so a hung or interactive binary can never delay a command. A missing binary is a normal outcome — it MUST be reported as `binary: "missing"` and MUST NOT raise or count an error metric. The declared targets and the consent decision MUST both be captured at command start, because some commands (`convert`) chdir into a throwaway project before emitting. The CLI's own version needs no attribute: the SDK stamps `sentry.release` = `cdktn-cli-<version>` on every metric.
- **FR-020** *(added 2026-09-09)*: The per-stack signal the HashiCorp payload carried MUST be restored as metrics rather than discarded. For synth/diff/deploy/destroy, each synthesized stack MUST yield one `cli.stack` carrying its backend kind, whether a cloud/remote block is present, the cdktn **library** version (CLI/library skew is a real support question) and the number of overrides, imports and moves; one `cli.stack.override` per overridden resource type with that type's count; and one `cli.stack.provider` per entry in `required_providers` with the provider's normalized public identity, its validated version constraint and whether the binding is `generated` (listed in `terraformProviders`) or `prebuilt`. Stacks that did not complete MUST be counted as `cli.stack.failed`. `get` and `init` MUST likewise count one item per generated binding / provider (`cli.get.provider`, `cli.get.module`, `cli.init.provider`), with the totals as attributes on the command metric. Names, ids, addresses, paths and messages MUST NOT be sent: imports and moves are reduced to counts, and a private-registry or non-registry identity is reduced to its kind.
- **FR-021** *(added 2026-09-09)*: Every value that reaches a metric attribute MUST be enumerated or validated first; no payload field is forwarded because it happens to be a scalar. Concretely: payload fields become attributes only through a per-command allow-list keyed by name and expected type (arrays and objects are never serialized); `language` must be one of the supported `LANGUAGES`; `backend` must be one of Terraform's built-in backend kinds, else `other`, and `unknown` when absent; `resource_type` must match the Terraform type grammar or be a `module.<kind>` form, else `other`; provider identities are only sent as public-registry `namespace/type` (other hosts become `private-registry`, paths and malformed input `other`) with each segment capped at 64 characters and 128 in total; module sources are only sent as public-registry `namespace/name/provider`, else `local`, `git`, `private-registry` or `other`; declared target ranges are canonicalized semver ranges, and provider constraints canonical Terraform constraints, with a prerelease or build identifier or an over-long value reducing the whole value to `invalid`; version-like values are reduced to MAJOR.MINOR.PATCH and omitted when they are not a release. A payload entry that cannot be validated MUST NOT take the whole run's command metric down with it. The fields the SDK stamps MUST be limited to the release, the constant environment `"production"`, the constant `server.address`, and the persistent random user id; no value from a `SENTRY_*` environment variable may reach an envelope.
- **FR-022** *(added 2026-09-09, absorbing PR #378 — fixes #360, #361)*: The CLI entrypoint MUST have exactly one failure path. The yargs `.fail()` handler MUST stay **synchronous** — it only records the failure, because yargs discards the handler's return value and an `await` there races Node's unhandled-rejection reporter — and the recorded failure MUST then be handled once by an awaited `reportFailure`, followed by a single `process.exit`. `Usage` errors MUST be printed as one line and not captured; `External` errors printed as one line and captured; anything else printed with its stack, captured, and followed by the debug-information block. A synchronous handler throw that bypasses `.fail()` MUST still be caught. The failed-command metric (`cli.command.error`) MUST be emitted **before** the single bounded flush, and every flush in the CLI MUST go through the one bounded helper in commons — no other module may call `Sentry.flush`/`Sentry.close` directly.
...
- **SC-001**: Zero outbound HTTP requests to HashiCorp endpoints when running any CLI command. *(Evidence revised 2026-09-09: this is now shown by the delivery oracle — a real client with a capturing transport, asserting on the raw envelope bytes — and by the CI bundle E2E against a local sink, which also scans the built bundle. The static source-scan test and the nock-canary test that previously stood as evidence were removed: they asserted nothing that a build or the sink assertions do not already assert.)*
...
- **SC-009** *(added 2026-09-09)*: The bundle E2E runs on **every build** in CI, not behind a gate or an opt-in flag: it builds a scratch bundle with a local-sink DSN, exercises the success, error, per-stack and entrypoint-crash paths, and fails the build when the expected envelope items do not arrive. It never rewrites the shipped bundle.
- **SC-010** *(added 2026-09-09)*: A deliberate refutation attempt finds nothing: with `SENTRY_ENVIRONMENT`, `SENTRY_TRACE` and `SENTRY_BAGGAGE` exported as LEAK markers and a stack synthesized whose name, imported resource id, provider host, provider organization and module path are all LEAK markers, none of those strings appears anywhere in the raw envelope bytes the sink recorded.
...
- [Review round 2 — restored usage signal, absorbed #378, single flush](research/2026-09-09-review-round-2.md) *(2026-09-09)*: the decisions of the second review round on PR #62 (Sentry stays on 10.x; the bundle E2E runs in CI instead of a gated jest test; one bounded flush helper; #378 absorbed; the restored payload emitted as per-item counts), the privacy findings from the refutation rounds and their fixes, and the test-review verdicts. Anchors FR-005's revision, FR-012, FR-014 and FR-019 – FR-022.
- [Asset pipeline telemetry evaluation](research/2026-09-09-asset-pipeline-telemetry-evaluation.md) *(2026-09-09)*: whether this telemetry should be extended to inform the asset-pipeline scope decision (#380/#371/#339). It contributes little; the one candidate metric (an allow-listed `cli.stack.resource`) is deferred as a follow-up and nothing is added to the `cdktn` library.
